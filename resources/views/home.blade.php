...
<head>
    ...
    <title>Portofolio - {{ $profile?->full_name ?? 'Portofolio' }}</title>
...
<body class="bg-white text-slate-800">
    ...
    <header class="gradient-bg min-h-screen flex flex-col">
        ...
        <div id="modalDev" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-600 hidden flex items-center justify-center p-4">
            <div class="bg-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl text-center animate-slide-up">
                ...
                <p class="text-slate-400 text-xs mb-8 leading-relaxed">Sabar ya Ari Kusuma, fitur <b>Contact</b> ini masih dalam tahap pengembangan tim kami.</p>
        ...
        <div id="modalPhoto" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] hidden flex items-center justify-center p-4">
            <div class="bg-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl animate-fade-in-up">
                <div class="text-center mb-6">
                    <div class="w-16 h-16 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">🖼️</div>
                    <h3 class="text-xl font-bold text-slate-800">Ganti Foto Profil</h3>
                    <p class="text-slate-400 text-xs">Pilih file gambar terbaikmu (Max 2MB)</p>
                </div>
                ...
                <form action="{{ route('profile.updatePhoto') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex gap-3">
                        <button type="button" onclick="closePhotoModal()" class="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-2xl hover:bg-slate-200 transition">Batal</button>
                        <button type="submit" class="flex-1 py-3 bg-blue-600 text-white font-bold rounded-2xl shadow-lg hover:bg-blue-700 transition">Upload</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="container mx-auto px-6">
            <div class="flex flex-col md:flex-row gap-16 items-start">
                ...
                <div class="w-full md:w-1/3 group animate-slide-in-left">
                    <div class="bg-white rounded-3xl p-8 shadow-[0_20px_50px_rgba(8,112,184,0.07)] border border-slate-100 transition-all group-hover:-translate-y-2 relative">
                        <button onclick="openPhotoModal()" class="absolute top-10 right-10 z-30 bg-white/90 p-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110">📸</button>
                        <div class="w-full aspect-square bg-slate-200 rounded-2xl mb-6 overflow-hidden relative">
                            <img src="{{ ($profile?->photo) ? asset('storage/photos/' . $profile->photo) . '?t=' . time() : asset('images/image.png') }}"
                                alt="Profile"
                                class="w-full h-full object-cover">
                        </div>
                        <h2 class="text-xl font-bold">{{ $profile?->full_name ?? 'Nama Belum Diatur' }}</h2>
                        <p class="text-blue-600 text-sm">{{ $profile?->profile_category ?? '-' }}</p>
                    ...
                    <div class="grid gap-8">
                        <div class="animate-fade-in-up animate-delay-200">
                            ...
                            <p class="text-slate-600 leading-relaxed italic">
                                @if($profile && $profile->vision && trim($profile->vision) !== '')
                                    "{{ $profile->vision }}"
                                @else
                        ...
                        <div class="animate-fade-in-up animate-delay-400">
                            ...
                            <p class="text-slate-600 leading-relaxed">
                                @if($profile && $profile->mission && trim($profile->mission) !== '')
                                    {{ $profile->mission }}
                                @else
    ...
    <script>
        ...
        // Menutup modal jika user klik di area luar modal
        window.onclick = function(event) {
            let modal = document.getElementById('modalPhoto');
